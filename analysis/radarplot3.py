...
def fetch_goal_row(participant_id, engine, goal_index=2):
...
def build_figure(participant_id, engine):
    goal_mapping = fetch_goal_row(participant_id, engine, goal_index=2)
    ...
    steps = []
    for t in range(2, 7):
        steps.append(dict(method="restyle", label=f"T{t}", args=[{"r": [values_for_time(t)], "name": [f"T{t}"]}, [1]]))

    goal_id = goal_mapping.get("GoalID")

    fig.update_layout(
        title=dict(
            text=(f"<b>Goal Traits Radar — Goal {goal_id}</b><br><sub>T1 baseline with slider comparison (T2–T6)</sub>"),
            x=0.5,
            xanchor="center",
        ...
        sliders=[dict(active=4, currentvalue=dict(prefix="Compare vs: ", font=dict(color="#c8d6f0")), pad=dict(t=40), steps=steps)],
        polar=dict(
            bgcolor="rgba(255,255,255,0)",
            radialaxis=dict(range=[1, 7], showticklabels=False, gridcolor="rgba(200,200,200,0.3)", gridwidth=1),
            angularaxis=dict(tickfont=dict(size=12, color="#c8d6f0"), gridcolor="rgba(200,200,200,0.3)"),
        ),
    )
